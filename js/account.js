...
const KEY_EXTRA = 'bc_user_extras'; // { <email>: {genre, goal} }
...
function renderShelf() {
  ...
  if (!box) return;
  if (!shelf.length) {
    box.innerHTML = '<p class="empty-msg">Your shelf is empty. Browse the <a href="3product.html">catalog</a> and click "Add to Shelf" 🔖</p>';
    return;
  }
  box.innerHTML = shelf.map((b, i) => `
    <div class="shelf-item">
      ...
      <div class="shelf-item-info">
        ...
        <a href="8reader.html?id=${encodeURIComponent(b.id)}" class="btn-small shelf-read-btn">📖 Read</a>
      </div>
      <button class="shelf-remove" data-i="${i}" title="Remove from shelf">✕</button>
    </div>
  `).join('');
}
...
function renderBooks() {
  ...
  if (!box) return;
  if (!books.length) {
    box.innerHTML = '<p class="empty-msg">No reading progress yet. Add a book below!</p>';
    return;
  }
    ...
    return `
      <div class="book-item">
        <div class="book-item-info">
          <h4>${escapeHtml(b.title)}</h4>
          <p class="book-item-author">${escapeHtml(b.author || 'Unknown')}</p>
          <p class="book-item-progress">${b.currentPage || 0} / ${b.totalPages || '?'} pages · ${pct}%</p>
          <div class="progress-bar"><div class="progress-fill" style="width:${pct}%"></div></div>
        </div>
        <div class="book-item-actions">
          <button class="btn-small" data-action="update" data-i="${i}">Update</button>
          <button class="btn-small danger" data-action="remove" data-i="${i}">Remove</button>
        </div>
      </div>`;
  }).join('');
}
...
/* ============ Profile render (uses Firebase user) ============ */
function renderProfileForUser(user) {
  ...
  if (user) {
    ...
    // Пароль не хранится — Firebase управляет им безопасно
    const aP = $('acc-password');
    if (aP) {
      aP.textContent = 'Managed by Firebase (secure)';
      aP.dataset.real = '';
      aP.dataset.shown = '0';
    }

    // Extras (genre / goal) — в localStorage по email
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Firebase calls this on load (restores session if user previously logged in)
